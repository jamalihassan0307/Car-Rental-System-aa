...
function displayCars(carsToDisplay) {
  ...
  carsToDisplay.forEach((car) => {
    ...
    card.innerHTML = `
            <img src="${car.image || "default-car-image.jpg"}" alt="${
      car.name
    }" onerror="this.src='default-car-image.jpg'">
            <h3>${car.name}</h3>
            <p>Model: ${car.model}</p>
...
function editCar(car) {
  ...
  details.innerHTML = `
        ...
        <form id="editCarForm" class="add-car-form">
            <div class="form-group">
                <label for="carName">Car Name</label>
                <input type="text" id="carName" name="name" value="${
                  car.name
                }" required>
            </div>
            <div class="form-group">
                <label for="carModel">Model</label>
                <input type="text" id="carModel" name="model" value="${
                  car.model
                }" required>
            </div>
            <div class="form-group">
                <label for="carPrice">Price per Day ($)</label>
                <input type="number" id="carPrice" name="price" value="${
                  car.price
                }" min="1" required>
            </div>
            <div class="form-group">
                <label for="carImage">Image URL</label>
                <input type="url" id="carImage" name="image" value="${
                  car.image
                }" required>
            </div>
            <div class="form-group">
                <label for="carDescription">Description</label>
                <textarea id="carDescription" name="description" required>${
                  car.description || ""
                }</textarea>
            </div>
            <div class="form-actions">
...
async function loadUserProfile() {
  try {
    ...
    document.getElementById("userRole").textContent = currentUser.isAdmin
      ? "Administrator"
      : "Regular User";
    document.getElementById("profileEmail").value = currentUser.email;
    document.getElementById("profilePhone").value = currentUser.phone || "";
    document.getElementById("profileAddress").value = currentUser.address || "";
    document.getElementById("accountType").value = currentUser.isAdmin
      ? "Administrator Account"
      : "Regular Account";
    document.getElementById("memberSince").value =
      currentUser.memberSince || new Date().toLocaleDateString();
